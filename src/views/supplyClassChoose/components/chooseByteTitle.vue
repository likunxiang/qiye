<template>
	<div>
		<div class="flex flex-center mb10" v-for="(item,index) in tableData" :key="index">
			<div class="filter-item-title mr10">{{item.name}}</div>
			<el-input ref="input" placeholder="请选择" v-model="checkedList[index].name" @keyup.enter.native="toSearch"
				style="width: 200px;" readonly @focus="openByte(item,index)">
				<i class="el-icon-search el-input__icon" slot="suffix" @click="toSearch">
				</i>
			</el-input>
		</div>
		<chooseByteContent v-if="isByte" @close="closeByte" :openRow="chooseRow" :guid="openRow.guid" :guidList='guidList'
			:byteIndex="byteIndex" @getByteGuid="getByteGuid" @getLastGuid="getLastGuid"></chooseByteContent>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			getByteGuid(data) {
				...
				this.guidList[this.byteIndex] = data.nameTreeGuid
				this.guidList.splice(this.byteIndex + 1, )
				this.guidListName[this.byteIndex] = data.name
				this.guidListName.splice(this.byteIndex + 1, )
				console.log(this.guidListName);
			...
			async getListBySceneGuid() {
				await getListBySceneGuid({
					sceneGuid: this.openRow.guid,
