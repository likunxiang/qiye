			...
			<el-row class="mt20">
				<el-radio-group v-model="radio" @change="getByteObj">
					<el-row style="margin-bottom: 5px;" v-for="(item,index) in tableData" :key="index">
						<el-radio :label="item.nameTreeGuid">{{item.name}}</el-radio>
					</el-row>
		...
		methods: {
			...
			getByteObj(val) {
				...
				for (let i in this.tableData) {
					if (this.tableData[i].nameTreeGuid === val) {
			...
			submit() {
				...
				this.$emit('getLastGuid', data.nameTreeGuid)
				this.close()
			},
			getTableData() {
				this.loading = true
				...
				if (this.byteIndex > 1) {
					this.getNameTreeContentByCTGuid(id)
				} else {
					this.getNameTreeContentByCTGuid()
				}
			},
			async getNameTreeContentByCTGuid(id) {
				let guid = ''
				if (id) {
					guid = id
				} else {
				...
				await getNameTreeContentByCTGuid({
					sceneGuid: this.guid,
					parentGuid: guid,
					name: this.searchVal,
					page: '1',
