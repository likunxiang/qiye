	...
	<el-dialog :title="title" :visible.sync="isOpen" width="700px" @close="beforeClose" append-to-body>
		<div style="padding-bottom: 40px;max-height: 700px;overflow-y: auto;">
			...
			<div v-if="contentFDCode == 'c00018'">
				<el-date-picker v-model="fieldValue" @change="changeDate" value-format="yyyy-MM-dd HH:mm:ss" type="datetime"
					format="yyyy-MM-dd HH" placeholder="选择日期时间">
				</el-date-picker>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async getByCode(contentFDCode, fieldGuid) {
				await getByCode({
					contentFDCode: contentFDCode || '',
					plateFieldGuid: fieldGuid,
					...
					if (res.OK == 'True') {
						if (res.Tag.length) {
							this.fieldContent = res.Tag[0].Table
						} else {
							this.fieldContent = []
						}
					}
		...
		created() {
			...
			let operation = this.fieldObj.operation || ''
			let contentFDCode = this.fieldObj.contentFDCode || ''
			let fieldFDCode = this.fieldObj.fieldFDCode || ''
			let fieldGuid = this.fieldObj.fieldGuid || ''
			...
			if (operation == '1' || operation == '2') {
				...
				if (contentFDCode == 'c00008') {
					// 行政区域国家/地区
				} else if (contentFDCode == 'c00009') {
					// 行政区域省级
				} else if (contentFDCode == 'c00010') {
					// 行政区域市级
				} else if (contentFDCode == 'c00011') {
					...
					// 行政区域镇级
				} else if (contentFDCode == 'c00007') {
					// 日期-年月日
				} else if (contentFDCode == 'c00017') {
					// 日期-年月日时分
				} else if (contentFDCode == 'c00018') {
					// 日期-年月日时
				} else if (contentFDCode == 'c00003') {
					// 地图
					this.getMyLocation()
				} else if (contentFDCode == 'c00013') {
					this.getByCode(contentFDCode, fieldGuid)
					contentFDCode = ''
					this.fieldValue = this.fieldObj.values[0].value
				} else {
					contentFDCode = ''
					this.getByCode(contentFDCode, fieldGuid)
					this.fieldValue = this.fieldObj.values[0].value
				}
				this.contentFDCode = contentFDCode
				console.log('contentFDCode',contentFDCode);
			}
			if (operation == '3') {
				this.fieldValue = this.fieldObj.values[0].value
			}
