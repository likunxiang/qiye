<template>
	<div>
		<div class="flex flex-center mb10" v-for="(item,index) in tableData" :key="index">
			...
			<el-input ref="input" placeholder="请选择" v-model="checkedList[index].content" @keyup.enter.native="toSearch"
				style="width: 200px;" readonly @focus="openByte(item,index)">
				<i class="el-icon-search el-input__icon" slot="suffix" @click="toSearch">
				</i>
			</el-input>
		</div>
		<chooseByteContent v-if="isByte" @close="closeByte" :openRow="chooseRow" :guidList='guidList'
